...
# Install program
{
    ...
    # Decompress package
    {
        ...
        if [ $? -ne 0 ] || [ ! -f "$TMP_DIR/$PROGRAM" ] || [ ! -d "$TMP_DIR/systemd" ]; then
    ...
    # Add systemd overlay
    {
        ...
        cat >/etc/systemd/system/nvidia-gridd.service.d/nvlts.conf <<EOF
[Service]
ExecStartPre=/opt/nvlts/nvlts -c /opt/nvlts/configs/vWS.json -g
EOF

        print_green "vWS profile installed at '/etc/systemd/system/nvidia-gridd.service.d/nvlts.conf'"
        print_green "If you are using vApps or vGaming, edit it then run 'systemctl restart nvidia-gridd'"
    }
}
